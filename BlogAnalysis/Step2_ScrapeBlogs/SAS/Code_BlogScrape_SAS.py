...
def save_to_excel(link, title, top_keywords):
    ...
    directory_path = f"sas_competitors/{category}/"
    try:
...
json_file_path = os.path.join(desktop_path, "Output_URL_SAS.json")
with open(json_file_path, "r") as file:
    data = json.load(file)

data = list(set(data))

if not os.path.exists("sas_competitors/"):
    os.makedirs("sas_competitors/")
...
# Save top keywords to Excel file
df = pd.DataFrame(list(top_keywords.items()), columns=['Keyword', 'Frequency'])
excel_file_path = os.path.join(desktop_path, "sas_competitors_top_100_keywords.xlsx")
df.to_excel(excel_file_path, index=False)
print(f"Top 100 keywords for all competitor blogs consolidated together saved to {excel_file_path}")
